...
export function fixListOfWagons(ids) {
  const [first, second, ...remaining] = ids;
  const fixedIds = [...remaining, first, second];
  return fixedIds;
  }
...
export function correctListOfWagons(ids, missingWagons) {
  ...
  const fixedIds =[first, ...missingWagons, ...remaining];
  return fixedIds;
}
...
export function extendRouteInformation(information, additional) {
  const updatedRouteInformation = {...information, ...additional};
...
export function separateTimeOfArrival(information) {
  ...
  const newObject = [timeOfArrival,remaining];
  return newObject;
}
